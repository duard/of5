...
export class UserGroupEntity extends BaseMysqlEntity {
  @Column({ name: 'st_description', type: 'varchar', comment: 'Nome do grupo de usuário' })
  description: string;

  @Column({ name: 'st_color_group', type: 'varchar', comment: 'Cor do grupo' })
  colorGroup: string;

  @Column({ name: 'dt_pass_expiration', type: 'timestamp', comment: 'Data de expiração da senha' })
  passExpiration: Date;

  // @Column({ name: 'updated_at', type: 'timestamp', default: () => 'CURRENT_TIMESTAMP' })
  // updatedAt: Date;

  @Column({ name: 'bn_pass_change', type: 'boolean', comment: 'Trocar senha? Sim/Não' })
  passChange: boolean;

  @Column({ name: 'in_pass_day_change', type: 'int', comment: 'Dias para troca de senha' })
  passDaysChange: number;

  @Column({ name: 'in_timeout', type: 'int', comment: 'Tempo de expiração' })
  timeout: number;

  @Column({ name: 'bn_mdi', type: 'boolean', comment: 'Utiliza mdi? Sim/Não' })
  mdi: boolean;

  @Column({ name: 'in_mid_max', type: 'int', comment: 'Numero máximo de janelas MDI' })
  mdiMax: number;
